...
async function verifyCategoryOwnership(categoryId: string, userId: string) {
    ...
    const workspace = await Workspace.findOne({ _id: category.workspaceId, ownerId: userId });
    if (!workspace) return { error: "Forbidden: You don't have access to this category's workspace", status: 403 };
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { category, error, status } = await verifyCategoryOwnership(id, userId);
        if (error) return errorResponse(null, status, error);

        return successResponse(category, 200, "Category fetched successfully");
    } catch (error: any) {
        return errorResponse(null, 500, error.message || "Failed to fetch category");
    }
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (body.name || body.type) {
            ...
            if (existingCategory) {
                return errorResponse(null, 400, "Category already exists with this name and type");
            }
        }
        ...
        return successResponse(updatedCategory, 200, "Category updated successfully");
    } catch (error: any) {
        return errorResponse(null, 500, error.message || "Failed to update category");
    }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return successResponse(deletedCategory, 200, "Category deleted successfully");
    } catch (error: any) {
        return errorResponse(null, 500, error.message || "Failed to delete category");
    }
}

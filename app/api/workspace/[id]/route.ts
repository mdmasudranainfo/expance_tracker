...
import { Category, Transaction, Wallet } from "@/src/backend/models";
...
export async function PUT(
  ...
  try {
    ...
    if (!updatedWorkspace)
      return errorResponse(null, 404, "Workspace not found");
...
export async function DELETE(
  ...
  try {
    ...
    if (workspace?.isPersonal)
      return errorResponse(null, 403, "Personal workspace cannot be deleted");

    // delete all related data (e.g., projects, tasks) if needed
    const deleteCategory = await Category.deleteMany({ workspaceId: id });
    const deleteWallet = await Wallet.deleteMany({ workspaceId: id });
    const deleteTransaction = await Transaction.deleteMany({ workspaceId: id });

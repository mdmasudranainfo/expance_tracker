...
async function verifyWalletOwnership(walletId: string, userId: string) {
  ...
  const workspace = await Workspace.findOne({
    _id: wallet.workspaceId,
    ownerId: userId,
  });
  if (!workspace)
    return {
      error: "Forbidden: You don't have access to this wallet's workspace",
      status: 403,
    };
  ...
  try {
    ...
    if (transactions.length > 0) {
      return errorResponse(
        null,
        400,
        "Cannot delete wallet: It is associated with existing transactions",
      );
    }
